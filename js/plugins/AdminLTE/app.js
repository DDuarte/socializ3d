...
function hash_change(newHash) {
    ...
    if (hash.length === 0) {
        ...
        var hash_parts = hash.slice(1).split("-"); // first character is #
        ...
        hash_parts.forEach(function (hash) {
            ...
            if (result[1] == "tab") {
                ...
                var tab = $(".nav-tabs-custom:has(a[href=\"#tab_" + tabName + "\"])");
                if (tab.length > 0) {
                    var top = tab.offset().top - $(".my_navbar").height();
...
$(function () {
    ...
    $('body').on('click', 'a[href^="#tab_"]', function (event) {
        var hash = event.target.hash;
        hash = hash.substr(hash.indexOf("_") + 1);
        Socializ3d.hash['tab'] = hash;
